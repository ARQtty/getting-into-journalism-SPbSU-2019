...
df <- read.csv("./postupleniye/2017/podano.csv", dec = ",")
...
#write.csv(x=jur, file="./postupleniye/2017/podano_jur.csv",sep = ";", quote = F)
...
ggplot(data = howMany, aes(x="", y=howMany$Group.1, fill=Name)) +
  ...
  geom_text(aes(y = Group.1/length(Group.1) + c(0, cumsum(Group.1)[-length(Group.1)]), 
                label = percent(Group.1/sum(Group.1))), size=5) +
...
postupilo_jur <- read.csv("./postupleniye/2017/postupilo.csv", sep = ";")
...
ggplot() +
  ...
  xlim(90, 300) +
  labs(title = "Баллы тех, кого приняли (зелёный) и не приняли (красный) в 2017 году",
       xlab = "Суммарный балл",
...
ggplot() +
  ...
  labs(title = "Баллы ЕГЭ тех, кого приняли (зелёный) и не приняли (красный) НА КОНТРАКТ в 2017 году",
...
ggplot()+
  ...
  labs(title="Зависимость баллов за творческий конкурс от баллов по ЕГЭ (рус+лит) в 2017 году",
